...
- middleware 子包覆盖率补至 100%（认证选项、角色/权限未认证、CSRF 跳过方法与
  Referer 回退、会话签名/轮换异常分支）；
- 移除 RotateSession 中不可达的冗余类型断言分支；
- 依赖升级：tracex v1.2.0（错误码大写）。

## [v1.9.1] - 2026-08-11

### 文档

- README / docs 与当前代码同步：标准中间件形态、家族插拔约定、发布规范更新；
- 清理过期规划文档与历史版本表述。
...
- 依赖升级：tracex v1.0.4、webx v1.5.6、cryptox v1.0.4、idgenx v1.4.4、confx v1.0.4、resiliencex v1.3.4、errx v1.5.7、logx v1.3.4、validx v1.2.6、testx v1.4.5 及三方最新版本。
...
- 依赖升级为 `github.com/lcylpzls/webx/v2 v2.0.1`（webx v2 主版本
  模块路径规范），authx 的 webx 集成包（middleware / oauth2 /
  示例）同步迁移导入路径。
...
- `authx.TraceHook` 链路追踪钩子（零依赖接口）+ token 包
  `WithTraceHook`：刷新令牌签发/校验/消费/轮换自动埋点
  （authx.operation 属性），由 tracex 等外部适配器接入；
- 刷新令牌函数增加可选的 `TraceOption` 变参（向后兼容）。

## [v1.0.0] - 2026-08-09

### 正式版声明

- authx 进入语义化版本稳定期：v1.x 起承诺不引入破坏性 API 变更，
  新增能力以向后兼容的方式演进；
- 九大模块（password/token/rbac/middleware/session/mfa/oauth2/audit/security）、
  双示例（full/login-demo）、九目标 fuzz、全套质量门禁全绿；

### 发布前收尾

- go.mod 工具链要求放宽至 `go 1.26.0`（不再强制最新 patch）；
- README 快速开始现代化：展示密码哈希、JWT 与中间件核心用法，
  并引导登录全链路示例；
- 会话 Cookie 超长防御补测试（>512 视为无会话并重建）。

## [v0.19.0] - 2026-08-09

### 终审与加固

- 周期清理任务增加 panic 恢复：单次清理函数 panic 不再导致
  后台任务静默终止，下次周期继续执行；
- `AsyncAuditor.Stop` 文档补充钩子阻塞语义；
- 依赖整理（go mod tidy，无残留变更）；
- 全量门禁复核：核心包 100% 覆盖、race、九目标 fuzz、
  vet/staticcheck、govulncheck（0 可调用漏洞）；
- 连续 10 轮全量测试无偶发竞态/死锁/阻塞。

### 版本线

- v0.1.0 - v0.19.0 全部完成并发布，按计划**停止于 v0.19.0**，不发布 v1.0.0；
- 最终状态：九大模块 + 双示例 + 九目标 fuzz + 全套质量门禁全绿。
