...
- 令牌：JWT 全套算法、刷新令牌旋转、撤销列表（规划中）；
- 授权：RBAC 角色/权限模型（规划中）；
- 集成：webx 认证/权限/CSRF 中间件（规划中）；
...
| 版本 | 内容 |
| --- | --- |
| v0.1.0 | password：Argon2id 哈希、校验、参数迁移 |
| v0.2.0 | token：JWT 全套、刷新令牌、撤销 |
| v0.3.0 | rbac + middleware：webx 认证/权限/CSRF |
